...
% Spanish edition. The preamble mirrors ../english/main.tex exactly except for
% babel, the T1 encoding, and the two box titles; the chapter files carry the
% same names and the same order so that the design-box numbering is identical
% in both editions, which is what lets Part III cross-reference by number.
%
...
% includefoot no es decorativo. Sin él, geometry mide `bottom` hasta el pie del
% BLOQUE DE TEXTO y deja el folio donde lo ponga \footskip, que eran 0,266
% pulgadas del corte: por encima del piso de 0,25 de KDP pero dentro del área de
% seguridad de 0,5 que dibuja una verificación de imprenta, de modo que el número
% de página quedaba marcado en cada apertura de parte y de capítulo. Con
% includefoot, `bottom` se mide hasta el pie del PIE DE PÁGINA. 0,53 y no 0,5
% porque la caja del glifo baja unas 0,024 pulgadas por debajo del borde nominal.
\usepackage[paperwidth=5in,paperheight=8in,includehead,includefoot,
            inner=0.6625in,outer=0.2875in,top=0.56in,bottom=0.53in]{geometry}
% Spanish typesetting. T1 matters: with the default OT1 encoding an accented
% word is built from an accent plus a letter and TeX will not hyphenate it at
% all, which on a 4.05in measure produces overfull lines throughout.
% es-noquoting and es-nolists stop babel making " active and re-styling lists,
% either of which would break the `rules` environment below.
...
\newenvironment{design}[1]
  {\par\addvspace{\medskipamount}%
   ...
   \begin{tcolorbox}[breakable,enhanced,colback=black!7,colframe=black!7,
     ...
     fontupper=\small]%
   \noindent{\normalsize\textbf{Dise\~no \thedesign.\ #1}}\par\nobreak\smallskip}
...
\newenvironment{ledger}[1]
  {\par\addvspace{\medskipamount}%
   \begin{tcolorbox}[breakable,enhanced,colback=black!7,colframe=black!7,
     ...
     left=6pt,right=6pt,top=6pt,bottom=6pt,
     fontupper=\small]%
   \noindent{\normalsize\textbf{El balance: #1}}\par\nobreak\smallskip}
  {\end{tcolorbox}\addvspace{\medskipamount}\noindent\ignorespacesafterend}
...
% Roto a mano en los dos puntos. Compuesto como una sola línea el título no cabe
% en la medida de 4,05 pulgadas y \maketitle lo parte solo, dejando "Futuro"
% huérfano en la segunda línea. El título inglés sí cabe en una línea y por eso
% allí no lleva el quiebre.
\title{Una perspectiva de Cuba:\\Pasado y Futuro}
...
\vspace*{\fill}
\begin{center}
{\itshape Para los que se quedaron, y los que se fueron.}
\end{center}
\vspace*{\fill}
\clearpage
...
\include{revolution}
\include{socialstate}
\include{sovieteconomy}
\include{internationalism}
