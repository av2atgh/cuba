...
% includefoot is not decorative. Without it geometry measures `bottom` to the
% foot of the TEXT BLOCK and leaves the folio to fall wherever \footskip puts
% it, which was 0.266in from the trim -- over KDP's 0.25in floor but inside the
% 0.5in safety area a print check draws, so the page number was flagged on every
% part and chapter opening. With includefoot, `bottom` is measured to the bottom
% of the FOOTER, so the clearance is stated rather than inherited. 0.53in and not
% 0.5in because the folio's glyph box extends ~0.024in below the nominal footer
% edge; measured, not guessed -- see the recipe under Building in README.md.
